...
// Calculator Results (Premium feature for authenticated users)
export const calculatorResults = pgTable(
  'calculator_results',
  {
    ...
    userId: varchar('user_id')
      .notNull()
      .references(() => users.id, { onDelete: 'cascade' }),
    calculatorType: text('calculator_type').notNull(), // bmi, tdee, body_fat, macros, etc.
    inputs: jsonb('inputs').notNull(), // Store all input values as JSON
    results: jsonb('results').notNull(), // Store calculated results as JSON
  ...
  (table) => [
    ...
    index('idx_calculator_results_user_type').on(table.userId, table.calculatorType),
    index('idx_calculator_results_created_at').on(table.createdAt),
  ]
);
...
export const calculatorResultsRelations = relations(calculatorResults, ({ one }) => ({
  user: one(users, { fields: [calculatorResults.userId], references: [users.id] }),
}));
...
export const userFitnessProfile = pgTable(
...
export const personalRecordHistory = pgTable(
...
export const userStrengthStandards = pgTable(
...
export const userMuscleFatigue = pgTable(
  'user_muscle_fatigue',
  {
    id: varchar('id')
      .primaryKey()
...
export const userMuscleVolume = pgTable(
  'user_muscle_volume',
  {
    id: varchar('id')
      .primaryKey()
...
export const workoutRecoveryLog = pgTable(
...
export const workoutSetLogs = pgTable(
  ...
  (table) => [
    ...
    index('idx_workout_set_logs_session_exercise').on(
      table.sessionId,
      table.exerciseId,
      table.setNumber
    ),
...
export const aiGeneratedWorkouts = pgTable(
  'ai_generated_workouts',
  {
    ...
    >(),
    cooldown:
      jsonb('cooldown').$type<{ exerciseName: string; duration: string; notes?: string }[]>(),
...
export const paymentPlans = pgTable(
  ...
  (table) => [index('idx_payment_plans_trainer_id').on(table.trainerId)]
);
...
export const clientIntake = pgTable(
  'client_intake',
  {
    id: varchar('id')
      .primaryKey()
...
export const achievementsRelations = relations(achievements, ({ many, one }) => ({
  ...
  exercise: one(exercises, {
    fields: [achievements.requirementExerciseId],
...
export const personalRecordHistoryRelations = relations(personalRecordHistory, ({ one }) => ({
  ...
  exercise: one(exercises, {
    fields: [personalRecordHistory.exerciseId],
    references: [exercises.id],
  }),
}));
...
export const userStrengthStandardsRelations = relations(userStrengthStandards, ({ one }) => ({
  ...
  exercise: one(exercises, {
    fields: [userStrengthStandards.exerciseId],
    references: [exercises.id],
  }),
}));
...
export const workoutSessionsRelations = relations(workoutSessions, ({ one, many }) => ({
  ...
  workoutAssignment: one(workoutAssignments, {
    fields: [workoutSessions.workoutAssignmentId],
