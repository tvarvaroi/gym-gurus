...
import { insertClientSchema, type InsertClient } from '@shared/validators';
import type { Client } from '@shared/schema';
import { apiRequest } from '@/lib/queryClient';
import { Loader2, Plus, Edit, Trash2, Info, Activity, Copy, Check, Key } from 'lucide-react';
import { z } from 'zod';
...
export function ClientFormModal({
  ...
  const copyCode = () => {
    navigator.clipboard.writeText(accessCodeModal.code).then(() => {
      setCopied(true);
  ...
  const createClientMutation = useMutation({
    mutationFn: (data: InsertClient) =>
      apiRequest('POST', '/api/clients', data).then((r) => r.json()) as Promise<{
        client: Client;
    ...
    onSuccess: (data: { client: Client; accessCode: string | null }) => {
      ...
      form.reset();
      if (data.accessCode) {
        setAccessCodeModal({ open: true, code: data.accessCode, clientName: data.client.name });
      } else {
        toast({
          title: 'Success',
  ...
  const onSubmit = useCallback(
    (data: ClientFormData) => {
      if (mode === 'create') {
        ...
        if (data.activityLevel !== client?.activityLevel)
          changedData.activityLevel = data.activityLevel;
        if (data.neckCircumference !== client?.neckCircumference)
          changedData.neckCircumference = data.neckCircumference as any;
        if (data.waistCircumference !== client?.waistCircumference)
          changedData.waistCircumference = data.waistCircumference as any;
        if (data.hipCircumference !== client?.hipCircumference)
          changedData.hipCircumference = data.hipCircumference as any;
  ...
  return (
    <>
      {/* Access Code reveal modal — shown after client creation */}
      <Dialog
        open={accessCodeModal.open}
        onOpenChange={(open) => !open && setAccessCodeModal((s) => ({ ...s, open: false }))}
      >
        <DialogContent
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              Share this access code with <strong>{accessCodeModal.clientName}</strong>.
              They&apos;ll use it to log in at <strong>/disciple-login</strong>.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="my-4 space-y-4">
            ...
            >
              <span className="text-3xl font-mono tracking-[0.25em] font-semibold text-primary">
            ...
            <p className="text-xs text-muted-foreground text-center">
              You can view or regenerate this code any time on the client&apos;s detail page.
            </p>
          </div>

          <DialogFooter>
            <Button
              onClick={() => setAccessCodeModal((s) => ({ ...s, open: false }))}
              className="w-full"
            >
              Done
            </Button>
              ...
              <div className="grid grid-cols-1 gap-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        Fitness Goal<span className="text-destructive ml-0.5">*</span>
                      </FormLabel>
                      <FormControl>
                        <Textarea
              ...
              <div className="space-y-4 pt-2">
                <div className="flex items-center gap-2 text-sm font-semibold text-primary">
                  ...
                  <span className="text-xs text-muted-foreground font-normal">
                    (Optional - for body fat % & calorie calculations)
                  </span>
                </div>
                ...
                <div className="space-y-3 pt-2">
                  <p className="text-xs text-muted-foreground">
                    ...
                    Body circumferences are used for calculating body fat percentage using the US
                    Navy Method
                  </p>
                    ...
                    <AlertDialogContent data-testid="dialog-delete-confirmation">
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          ...
                          undone. All associated workout plans, progress data, and messages will be
                          permanently removed.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <DialogFooter>
