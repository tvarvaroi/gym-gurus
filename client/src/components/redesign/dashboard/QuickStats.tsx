...
export function QuickStats({ stats, strengthSummary, gamification }: QuickStatsProps) {
  ...
  return (
    <motion.div {...animProps}>
      ...
      <div className="flex gap-2.5 overflow-x-auto snap-x snap-mandatory scrollbar-hide -mx-4 px-4 md:hidden">
        {items.map((item) => (
          <Link key={item.label} href={item.href}>
            <a className="snap-start flex-shrink-0 w-[120px] bg-card rounded-2xl border border-border/20 p-3 cursor-pointer hover:border-primary/30 transition-colors">
              <span className="text-2xl font-bold tabular-nums leading-none block">
                {item.value}
              </span>
              <span className="text-[11px] uppercase tracking-wider text-muted-foreground/50 font-medium block mt-1.5">
                {item.label}
              </span>
              <span className="text-[10px] text-muted-foreground/30 block">{item.sub}</span>
            </a>
          </Link>
        ))}
      </div>
      ...
      <div className="hidden md:grid md:grid-cols-4 gap-3">
        {items.map((item) => (
          <Link key={item.label} href={item.href}>
            <a className="bg-card rounded-2xl border border-border/20 p-4 cursor-pointer hover:border-primary/30 transition-colors group">
