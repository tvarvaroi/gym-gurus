        ...
        <div className="flex gap-1">
          {Array.from({ length: 7 }).map((_, i) => {
            ...
            return (
              ...
              >
                <span
                  className={`text-[10px] uppercase tracking-wider ${isToday ? 'text-primary font-bold' : 'text-muted-foreground/40'}`}
                >
                  {dayName}
                </span>
                <span
                ...
                <div className="mt-1.5 h-4 flex items-center justify-center">
                  {hasWorkout ? (
                    <div
                      className={`w-3.5 h-3.5 rounded-full ${getTypeDotColor(workoutType)} flex items-center justify-center`}
                    >
