          ...
          {alerts.map((alert) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={`w-2 h-2 rounded-full shrink-0 ${alert.severity === 'urgent' ? 'bg-red-500' : 'bg-amber-500'}`}
                />
                <div>
...
const Dashboard = memo(() => {
  ...
  const trainerAchievements = useMemo(
    () => [
      ...
      },
      {
        icon: Star,
        title: '10 Clients',
        description: 'Reach 10 active clients',
        unlocked: (dashboardStats?.activeClients || 0) >= 10,
      },
      {
        icon: Target,
        title: '50 Sessions',
        description: 'Complete 50 sessions',
        unlocked: totalSessionsCompleted >= 50,
      },
      {
        icon: Award,
        title: '100 Workouts',
        description: 'Create 100 workout plans',
        unlocked: (dashboardStats?.totalWorkouts || 0) >= 100,
      },
    ],
    [
      currentStreak,
  ...
  const stats = useMemo(
    () => [
      ...
      },
      {
        ...
        value: loadingStats
          ? '--'
          : (dashboardStats?.totalWorkouts || workouts?.length || 0).toString(),
        icon: Dumbbell,
        trend: 'Active plans',
        change: '',
        changeType: 'increase',
  ...
  const recentActivities = useMemo(() => {
    return (
      dashboardStats?.recentActivity?.map((activity: any) => ({
        ...
        {
          client: 'Welcome!',
          action: 'Start by adding your first client',
          time: 'Just now',
          icon: Users,
        },
      ]
  ...
  const showWelcomeModal =
    onboardingProgress && !onboardingProgress.welcomeModalCompleted && !welcomeDismissedLocally;
  const showChecklist =
    onboardingProgress?.welcomeModalCompleted &&
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center space-y-4 max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">
            {(clientsError || statsError)?.message ||
  ...
  return (
    <div className="space-y-6">
      ...
      />
      {showChecklist && (
        <SetupChecklist items={checklistItems} onDismiss={handleChecklistDismiss} />
      )}
      <ClientFormModal
        mode="create"
        trainerId={user?.id || ''}
