  ...
  .refine(
    (data) => {
      ...
      if (
        ...
        data.SESSION_SECRET === 'dev-secret-key-change-in-production-min-32-chars'
      ) {
        return false;
      }
      return true;
    ...
    {
      message:
        'SESSION_SECRET must be set to a secure value in production (not the default dev secret)',
      path: ['SESSION_SECRET'],
    }
  )
  .refine(
    ...
    {
      message:
        'At least one database connection method must be configured (DATABASE_URL, REPLIT_DB_URL, or PG* variables)',
      path: ['DATABASE_URL'],
    }
  );
